...
def draw_line(img, p1, p2):
    ...
    cv2.line(img, xy1, xy2, [0,0,255])
    # Line thickness of 2 px 
    # breakpoint()
    cv2.putText(img, 'too close', tuple(np.mean([xy1,xy2],axis=0,dtype=int)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, [255,255,255], 1) 
...
for group in output:
    ...
    if distance < 183: # if less than 183 cm or 6 feet apart
        ...
        p2 = group[3]
        # breakpoint()
        cv2.line(frame, tuple(p1[:2]+p1[2:]//2), tuple(p2[:2]+p2[2:]//2), [0,0,255])
